...
public class FittingAlignment {
	...
	public static void main(String[] args) {
		...
		// "Free taxi rides" to any point in the word v
		...
		for(int i=1; i<=vLen; i++) {
			for(int j=1; j<wLen; j++) {
				...
				if(s[i][j] == s[i-1][j]-mismatchIndelPenalty)
		...
		// free taxi rides going up for rightmost column of s[][]
		for(int i=1, j=wLen; i<=vLen; i++) {
			s[i][j] = max(s[i-1][j], s[i][j-1] - mismatchIndelPenalty, s[i-1][j-1]+diagonal[i-1][j-1]);
	...
	static void computeLCSRec(int[][] backtrack, String v, String w, int i, int j, StringBuilder $v, StringBuilder $w) {
		...
		if(j==0 || backtrack[i][j]==Direction.UP.ordinal()) {
			...
			if (j!= w.length() && j!=0) { // if it's the first or last column, don't add any letters; just a free taxi ride
